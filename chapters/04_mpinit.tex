...
Doeblin's minorization condition is established by exploiting the bounded support of $U[t]$ together with the stochastic noise injected by the i.i.d.\ inputs. Once the minorization condition holds, classical results on Markov chains imply existence and uniqueness of the stationary distribution as well as exponential decay of TV distance. A complete proof is provided in the supplementary discussion of the underlying paper; the argument adapts the continuous-time treatment of \cite{dumont:2017} to the discrete-time recurrence in \cref{eq:lif_discrete_potential,eq:lif_discrete_reset}.
\end{proof}
...
\label{lemma:1}
Among all constants $c \in \mathbb{R}$, the choice $c = \mathbb{E}[\pi]$ minimizes the expected square difference $\mathbb{E}[(\pi-c)^{2}]$.
\end{lemma}

\begin{proof}
Standard variance decomposition: $\mathbb{E}[(\pi-c)^{2}] = \mathrm{Var}(\pi) + (\mathbb{E}[\pi] - c)^{2}$, which is minimized at $c=\mathbb{E}[\pi]$.
\end{proof}
