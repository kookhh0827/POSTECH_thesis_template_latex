...
\begin{figure}[h]
  ...
  \begin{subfigure}[b]{0.99\linewidth}
    ...
    \caption{ResNet-19 on CIFAR-100}
  \end{subfigure}
  \par\medskip
  \begin{subfigure}[b]{0.99\linewidth}
...
In \cref{ch:trsg} we introduced TrSG using the rectangular surrogate as the primary example. \Cref{sec:exp_trsg} extended the experiments to the triangular surrogate. We now demonstrate TrSG's generality by evaluating it with the Arctan-based~\cite{fang:2021} and Sigmoid-based~\cite{wu:2018} surrogates.
...
\Cref{fig:acc_different_atan} shows that TrSG consistently outperforms AS-SG and RS-SG, regardless of the initial threshold or time constant. This robust behavior parallels the rectangular and triangular results in \cref{sec:exp_trsg}, indicating that TrSG's threshold-robustness extends across all common surrogate-gradient shapes.
...
\begin{figure}[h]
  ...
  \begin{subfigure}{0.99\linewidth}
    ...
    \caption{Arctan: $\tau=2.0$, $V_{\text{thr}}\in\{0.5,1.0,2.0\}$}
  \end{subfigure}
  \par\medskip
  \begin{subfigure}{0.99\linewidth}
    \centering
    \includegraphics[width=0.99\linewidth]{fig/acc_different_tau_atan.pdf}
    \caption{Arctan: $V_{\text{thr}}=1.0$, $\tau\in\{1.33,2.0,3.0\}$}
  \end{subfigure}
  \par\medskip
  \begin{subfigure}{0.99\linewidth}
    \centering
    \includegraphics[width=0.99\linewidth]{fig/acc_different_thr_sigmoid.pdf}
    \caption{Sigmoid: $\tau=2.0$, $V_{\text{thr}}\in\{0.5,1.0,2.0\}$}
  \end{subfigure}
  \par\medskip
  \begin{subfigure}{0.99\linewidth}
    \centering
    \includegraphics[width=0.99\linewidth]{fig/acc_different_tau_sigmoid.pdf}
    \caption{Sigmoid: $V_{\text{thr}}=1.0$, $\tau\in\{1.33,2.0,3.0\}$}
  \end{subfigure}
  \caption{\textbf{Accuracy under different initial values of $\tau$ and $V_{\text{thr}}$ when learning both parameters, using the Arctan and Sigmoid surrogate functions.} (Top two) Arctan results; (Bottom two) Sigmoid results. We compare AS-SG, RS-SG, and TrSG on ResNet-19 (CIFAR-100, $T=4$), evaluating both soft- and hard-reset LIF neurons.}
...
To complement the experiments in \cref{sec:exp_trsg} (which varied initial $V_{\text{thr}}$), we run additional tests with the initial threshold fixed at $V_{\text{thr}}=1.0$ while varying the initial time constant $\tau\in\{1.33,2.0,3.0\}$. \Cref{fig:acc_different_tau_rect_tri} reports accuracy under the rectangular (top) and triangular (bottom) surrogates, comparing AS-SG, RS-SG, and TrSG with both soft and hard resets. TrSG consistently outperforms AS-SG and RS-SG across all initial $\tau$ values, maintaining high accuracy in every setting and reaffirming the robustness observed in \cref{sec:exp_trsg}.
...
\begin{figure}[h]
  ...
  \begin{subfigure}{0.99\linewidth}
    ...
    \caption{Triangular surrogate}
    \label{fig:acc_different_tau_tri}
  \end{subfigure}
  \caption{\textbf{Accuracy under different initial $\tau$ values with $V_{\text{thr}}=1.0$.} We compare AS-SG, RS-SG, and TrSG under (a) rectangular and (b) triangular surrogates, each with both soft and hard resets. Initial $\tau\in\{1.33,2.0,3.0\}$. ResNet-19 on CIFAR-100, $T=4$.}
